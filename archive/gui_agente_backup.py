...
class AgenteGUI:
    def __init__(self, root):
        self.root = root
        self.root.title("Memorae - Asistente AI Local")
    ...
    def create_widgets(self):
        ...
        # Título
        title_label = tk.Label(main_frame, text="🤖 Memorae - Asistente AI", 
                              font=('Arial', 16, 'bold'), 
                              bg='#2b2b2b', fg='#4a9eff')
    ...
    def create_config_tab(self):
        ...
        settings_frame = ttk.LabelFrame(config_frame, text="Configuraciones del Agente")
...
Para respuesta simple, solo texto natural."""
        ...
        try:
            while True:
                ...
                if msg_type == "response":
                    self.add_message_to_chat(f"🤖 Memorae: {content}", "agent")
                elif msg_type == "error":
    ...
    def load_chat_history(self):
        ...
        for user_msg, agent_msg in history:
            ...
            self.add_message_to_chat(f"🤖 Memorae: {agent_msg}", "agent")
    ...
    def refresh_tasks(self):
        ...
        for task in tasks:
            # Formatear fecha
            try:
                fecha = datetime.datetime.fromisoformat(task[2]).strftime("%Y-%m-%d %H:%M")
            except:
                fecha = task[2]
    ...
    def update_stats(self):
        ...
        last_interaction = last_interaction[0] if last_interaction else "Nunca"
        
        conn.close()
        
        stats_text = f"""📊 ESTADÍSTICAS DEL AGENTE
