"""
JDMMItAsistente Optimizado
Versión mejorada del asistente AI con MySQL y mejor arquitectura
"""
...
from config import ASSISTANT_NAME, LOG_FILE, DB_CONFIG
...
try:
    from config import MODEL, DB_CONFIG, CREDENTIALS_PATH, EMAIL_SMTP, EMAIL_PORT, EMAIL_USER, EMAIL_PASS, WHATSAPP_NUMBER, GOOGLE_SPEECH_LANGUAGE, ASSISTANT_NAME
...
class DatabaseManager:
    """Gestor de base de datos MySQL con contexto y mejor manejo de errores"""
    ...
    def get_connection(self):
        ...
        try:
            ...
            logger.error(f"Error en base de datos MySQL: {e}")
            if conn:
                conn.rollback()
            raise
        finally:
    ...
    def init_db(self):
        """Inicializar base de datos MySQL con mejor esquema"""
        try:
            with self.get_connection() as conn:
                cursor = conn.cursor()
                
                # Tabla historial (MySQL sintaxis)
                cursor.execute('''
    ...
    def save_interaction(self, user_input: str, agent_output: str, session_id: str = None, tokens_used: int = 0) -> int:
        ...
        with self.get_connection() as conn:
            ...
            cursor.execute(
                'INSERT INTO historial (timestamp, usuario_input, agente_output, session_id, tokens_used) VALUES (%s, %s, %s, %s, %s)',
    ...
    def get_recent_history(self, limit: int = 5) -> List[Interaction]:
        ...
        with self.get_connection() as conn:
            cursor = conn.cursor()
            cursor.execute(
                'SELECT id, timestamp, usuario_input, agente_output FROM historial ORDER BY id DESC LIMIT %s',
                (limit,)
            )
            rows = cursor.fetchall()
            return [
                Interaction(id=row[0], timestamp=str(row[1]), usuario_input=row[2], agente_output=row[3])
    ...
    def save_task(self, task: Task) -> int:
        ...
        with self.get_connection() as conn:
            ...
            cursor.execute(
                'INSERT INTO tareas (descripcion, fecha, prioridad, timestamp, categoria, notas) VALUES (%s, %s, %s, %s, %s, %s)',
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        with self.get_connection() as conn:
            ...
            stats['last_interaction'] = str(result[0]) if result else "Nunca"
...
class LLMManager:
    ...
    def _build_prompt(self, user_input: str, context: str) -> str:
        ...
        return f"""Eres {ASSISTANT_NAME}, un asistente AI personal inteligente y útil. Tu objetivo es ayudar al usuario con sus tareas diarias, recordatorios y preguntas.
...
class JDMMItAsistenteCLI:
    """Interfaz de línea de comandos mejorada para JDMMItAsistente"""
    ...
    def run(self):
        ...
        logger.info(f"🚀 Iniciando {ASSISTANT_NAME} CLI optimizado")
        
        # Mostrar información inicial
        stats = self.db_manager.get_stats()
        print(f"""
🤖 {ASSISTANT_NAME} - Asistente AI Personal
{'='*50}
...
""")
        ...
        try:
            ...
            print(f"👋 ¡Hasta luego! {ASSISTANT_NAME} se está cerrando...")
